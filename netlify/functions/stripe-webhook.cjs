/**
 * netlify/functions/stripe-webhook.js  (Labrats — email + Sanity order log)
 *
...
const FROM = process.env.ORDER_EMAIL_FROM || 'Labrats <orders@labrats.uk>';
...
const MERCHANT_FROM = process.env.NOTIFICATION_FROM || 'Labrats <orders@labrats.uk>';
    ...
    <tr><td align="center">
      <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:${C.cardBg};border:1px solid ${C.teal};">
        <tr><td style="background:${HEADER_BG};padding:22px 28px;text-align:center;">
          ${LOGO_URL
            ? `<img src="${LOGO_URL}" alt="Labrats" width="240" style="max-width:240px;width:240px;height:auto;display:inline-block;border:0;" />`
            : `<span style="font-family:Arial,Helvetica,sans-serif;font-size:20px;font-weight:800;letter-spacing:3px;color:${C.teal};text-transform:uppercase;">Labrats</span>`}
        </td></tr>
        <tr><td style="padding:32px 28px 8px;">
          ...
          <p style="margin:0 0 20px;font-family:Arial,Helvetica,sans-serif;font-size:15px;line-height:1.6;color:${C.text};">
            Mission accepted. Your gear is heading into production — we'll send tracking the moment it breaks out of the lab.
          </p>
        </td></tr>
        ...
        <tr><td style="padding:28px;">
          <p style="margin:0;font-family:Arial,Helvetica,sans-serif;font-size:12px;line-height:1.6;color:${C.muted};border-top:1px solid ${C.border};padding-top:16px;">
            Labrats — printed &amp; shipped on demand. Questions? Just reply to this email.
          </p>
        </td></tr>
      </table>
    </td></tr>
  ...
  try {
    const res = await fetch(RESEND_URL, {
      ...
      body: JSON.stringify({
        from: FROM,
        to,
        subject: 'Your Labrats order is confirmed',
...
async function sendMerchantEmail(session, lineItems, status, printfulOrderId) {
  ...
  const subject = `${failed ? '\u26A0 ACTION NEEDED \u2014 ' : (hasInhouse ? '\uD83D\uDCE6 MAKE \u2014 ' : '')}New Labrats order #${ref} \u2014 ${gbp(session.amount_total)}`;
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0"><tr><td align="center">
      <table role="presentation" width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;background:#fff;border-radius:8px;padding:28px;">
        <tr><td>
          ...
          <p style="margin:0 0 16px;color:#666;font-size:13px;">Labrats &middot; ${esc(new Date().toLocaleString('en-GB'))}</p>
          ${banner}
...
async function saveOrder(session, lineItems, status, printfulOrderId) {
  ...
  const items = (lineItems.data || []).map((li, i) => {
    ...
    return {
      ...
      colour: inhouse ? (meta(li, 'wallart_format') || '') : (meta(li, 'labrats_colour') || ''),
      size: inhouse ? (meta(li, 'wallart_size') || '') : (meta(li, 'labrats_size') || ''),
